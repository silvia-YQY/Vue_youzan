          ...
          <div class="block-item">
            ...
            <div class="select-group">
              <select class="js-province-selector" v-model.trim='provinceValue' >
                ...
                <option :key="p.value" :value="p.value" v-for="p in addressData.list" >{{p.label}} </option>

              </select>
              <select class="js-city-selector" v-model.trim='cityValue'>
                <option value="-1">选择城市</option>
                <option :key="c.value" :value="c.value" v-for="c in cityList" >{{c.label}} </option>
              </select>
              <select class="js-county-selector" name="area_code" data-code="" v-model.trim='districtValue' >
                <option value="-1">选择地区</option>
                <option :key="d.value" :value="d.value" v-for="d in districtList" >{{d.label}} </option>
              </select>
            </div>
          </div>
          <div class="block-item">
            <label>详细地址</label>
            <input type="text" placeholder="街道门牌信息" name="address" v-model.trim="address" maxlength="100">
          </div>
        </div>
      </div>
